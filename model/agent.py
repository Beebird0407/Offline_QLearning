...
from .qmamba import QMamba, QEnsemble
...
class QMAgent:
    ...
    def from_checkpoint(cls, path: str, device: str = 'cpu', force_cpu: bool = False, **model_kwargs) -> 'QMAgent':
        ...
        # Auto-detect Mamba backend from state dict keys
        uses_mamba = any('in_proj' in k or 'A_log' in k for k in state_dict.keys())
        if uses_mamba and not force_cpu:
        ...
        model_kwargs.setdefault('force_cpu', force_cpu or not uses_mamba)
...
class QEnsembleAgent:
    """Agent wrapping a trained QEnsemble model for inference."""

    def __init__(
        self,
        model: QEnsemble,
        device: str = 'cpu',
        inference_mode: str = 'pessimistic',
    ):
        self.model = model.to(device)
        self.device = device
    ...
    def from_checkpoint(cls, path: str, device: str = 'cpu',
                        inference_mode: str = 'pessimistic',
                        **model_kwargs) -> 'QEnsembleAgent':
        checkpoint = torch.load(path, map_location=device, weights_only=False)
        config = checkpoint.get('config', {})
        trainer_cfg = checkpoint.get('trainer_config', {})
        inference_mode = inference_mode or trainer_cfg.get('inference', 'pessimistic')
        ...
        model_kwargs.setdefault('n_layers', config.get('n_layers', 1))
        model_kwargs.setdefault('num_hidden_mlp', config.get('num_hidden_mlp', 32))
        model_kwargs.setdefault('n_members', config.get('n_members', 3))
        model_kwargs.setdefault('force_cpu', not uses_mamba)
        model_kwargs.setdefault('base_seed', config.get('base_seed', 42))

        model = QEnsemble(**model_kwargs)
        state_dict = checkpoint['model_state_dict']
        state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
        model.load_state_dict(state_dict)
        model = model.to(device)
